/*
Copyright 2025 Grafana
*/

// Code generated by generate-observed; DO NOT EDIT.

package oncall

import (
	"context"

	"github.com/crossplane/crossplane-runtime/v2/pkg/resource"
	"github.com/hashicorp/terraform-plugin-framework/path"
	"github.com/hashicorp/terraform-plugin-framework/tfsdk"
	"github.com/hashicorp/terraform-plugin-go/tftypes"

	v1alpha1 "github.com/grafana/crossplane-provider-grafana/v2/apis/observed/oncall/v1alpha1"
	tfdatasource "github.com/grafana/crossplane-provider-grafana/v2/internal/controller/namespaced/observed/tfdatasource"
)

var UserSpec = tfdatasource.Spec{
	DataSourceName: "grafana_oncall_user",
	ManagedKind:    v1alpha1.User_GroupVersionKind,
	NewManaged:     func() resource.Managed { return &v1alpha1.User{} },
	Read: tfdatasource.NewFrameworkReadFn(
		newDSUser,
		func(mg resource.Managed) map[string]tftypes.Value {
			cr := mg.(*v1alpha1.User)
			attrs := map[string]tftypes.Value{}
			attrs["username"] = tftypes.NewValue(tftypes.String, cr.Spec.ForProvider.Username)
			return attrs
		},
		func(mg resource.Managed, state tfsdk.State) {
			cr := mg.(*v1alpha1.User)
			{
				var v *string
				if diags := state.GetAttribute(context.Background(), path.Root("email"), &v); !diags.HasError() && v != nil {
					cr.Status.AtProvider.Email = v
				}
			}
			{
				var v *string
				if diags := state.GetAttribute(context.Background(), path.Root("role"), &v); !diags.HasError() && v != nil {
					cr.Status.AtProvider.Role = v
				}
			}
		},
	),
}
