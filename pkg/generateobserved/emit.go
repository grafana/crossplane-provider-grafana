...
func generateLegacyFactories(cfg Config, dsList []*dsInfo, ci CategoryRule) string {
	...
	fmt.Fprintf(&b, "import (\n")
	fmt.Fprintf(&b, "\t%s %q\n", cfg.TFProviderImportAlias, cfg.TFProviderImport)
	fmt.Fprintf(&b, "\tsdkschema \"github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema\"\n")
	fmt.Fprintf(&b, ")\n\n")
	...
	for _, ds := range legacyDSList {
		fmt.Fprintf(&b, "var legacyDS%s *sdkschema.Resource\n", ds.kindName)
	}
	b.WriteString("\n")

	fmt.Fprintf(&b, "func init() {\n")
	fmt.Fprintf(&b, "\tp := %s\n", cfg.TFLegacyProviderFunc)
	fmt.Fprintf(&b, "\tfor name, ds := range p.DataSourcesMap {\n")
	fmt.Fprintf(&b, "\t\tswitch name {\n")
	for _, ds := range legacyDSList {
		fmt.Fprintf(&b, "\t\tcase %q:\n", ds.tfName)
		fmt.Fprintf(&b, "\t\t\tlegacyDS%s = ds\n", ds.kindName)
	}
	fmt.Fprintf(&b, "\t\t}\n")
	fmt.Fprintf(&b, "\t}\n")
	fmt.Fprintf(&b, "}\n")
